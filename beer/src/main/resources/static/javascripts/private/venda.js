Beer.Venda = (function() {
	...
	function onValorFreteAlterado(evento) {
		this.valorFrete = Beer.recuperarValor($(evento.target).val());
	}
	
	function onValorDescontoAlterado(evento) {
		this.valorDesconto = Beer.recuperarValor($(evento.target).val());
	}
	
	function onValoresAlterados() {
		var valorTotal = numeral(this.valorTotalItens).value() + numeral(this.valorFrete).value() - numeral(this.valorDesconto).value();
		this.valorTotalBox.html(Beer.formatarMoeda(valorTotal));
...
$(function() {
	var autocomplete = new Beer.Autocomplete();
	autocomplete.iniciar();
	
	var tabelaItens = new Beer.TabelaItens(autocomplete);
	tabelaItens.iniciar();
	
	var venda = new Beer.Venda(tabelaItens);
	venda.iniciar();
});