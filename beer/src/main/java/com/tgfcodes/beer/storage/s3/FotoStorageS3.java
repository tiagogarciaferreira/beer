package com.tgfcodes.beer.storage.s3;
...
public class FotoStorageS3 implements FotoStorage {
	...
	private static final String BUCKET = "awbeer";
	...
	public String getUrl(String foto) {
		if (!StringUtils.isEmpty(foto)) {
			return "https://s3.amazonaws.com/awbeer/" + foto;
		}
		return null;
	}
	...
	private ObjectMetadata enviarFoto(String novoNome, MultipartFile arquivo, AccessControlList acl) throws IOException {
		...
		amazonS3.putObject(new PutObjectRequest(BUCKET, novoNome, arquivo.getInputStream(), metadata).withAccessControlList(acl));
		return metadata;
	}
	...
	private void enviarThumbnail(String novoNome, MultipartFile arquivo, AccessControlList acl)	throws IOException {
		...
		amazonS3.putObject(new PutObjectRequest(BUCKET, THUMBNAIL_PREFIX + novoNome, is, thumbMetadata).withAccessControlList(acl));
	}
